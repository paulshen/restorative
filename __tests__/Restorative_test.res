...
@val @module("react-dom/test-utils")
...
describe("Restorative", () => {
  ...
  describe("Subscriptions", () => {
    test("it calls subscriptions", () => {
      ...
      let {getState, subscribe, dispatch} = createStore(initialState, reducer)
      subscribe(MockJs.fn(subscription1), ()) |> ignore
      subscribe(MockJs.fn(subscription2), ()) |> ignore
      dispatch(Increment)
      let state = getState()
      expect((MockJs.calls(subscription1), MockJs.calls(subscription2))) |> toEqual((
        [state],
        [state],
      ))
    })
    ...
    test("it respects custom areEqual", () => {
      ...
      let {subscribe, dispatch} = createStore(initialState, reducer)
      subscribe(MockJs.fn(subscription1), ~areEqual=(a, b) => a.count == b.count, ()) |> ignore
      subscribe(MockJs.fn(subscription2), ~areEqual=(a, b) => a.name == b.name, ()) |> ignore
  ...
  describe("Selectors", () => {
    test("it selects state slice", () => {
      ...
      let {subscribeWithSelector, dispatch} = createStore(initialState, reducer)
      subscribeWithSelector(state => state.count, MockJs.fn(subscription), ()) |> ignore
    ...
    test("it respects slice equality", () => {
      ...
      let {subscribeWithSelector, dispatch} = createStore(initialState, reducer)
      subscribeWithSelector(state => state.count, MockJs.fn(countSubscription), ()) |> ignore
      subscribeWithSelector(state => state.static, MockJs.fn(staticSubscription), ()) |> ignore

      dispatch(Increment)
      expect((MockJs.calls(countSubscription), MockJs.calls(staticSubscription))) |> toEqual((
        [1],
        [],
      ))
    })
  ...
  describe("Hooks", () => {
    test("useStore", () => {
      ...
      expect((calls1[Array.length(calls1) - 1], calls2[Array.length(calls2) - 1])) |> toEqual((
        initialState,
    ...
    test("useStoreWithSelector", () => {
      let {useStoreWithSelector, dispatch} = createStore(initialState, reducer)

      let renderMock = JestJs.fn(ignore)
      let renderFn = MockJs.fn(renderMock)
      let useStoreWithSelector = () => useStoreWithSelector(state => state.count, ())
      render(<TestComponent useStore=useStoreWithSelector dispatch render=renderFn />) |> ignore
      ...
      expect((calls1, calls2, calls3)) |> toEqual(([0, 0], [0, 0, 1], [0, 0, 1]))
    })
  })
})
